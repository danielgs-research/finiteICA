...
load('sim_data/sim_data_start_2021_08_06_08:19_end_2021_08_06_08:29_sim_total_corr_pure_ICA.mat');%P=2
...
ranking_QICA = [];
ranking_sa4ica = [];
load('sim_data/sim_data_start_2021_08_06_08:40_end_2021_08_06_09:03_sim_total_corr_pure_ICA.mat');%P=3
lines = length(n_sources);
columns = length(distributions_names);
for it_lines=1:lines
    for it_columns=1:columns
...
load('sim_data/sim_data_start_2021_08_06_12:41_end_2021_08_06_21:27_sim_total_corr_pure_ICA.mat');%P=5
lines = length(n_sources);
columns = length(distributions_names);
for it_lines=1:lines
    for it_columns=1:columns
...
fprintf('P=3 and P=5 - Results - %d experiments:\n', length(ranking_america));
fprintf('AMERICA:\t%.2f (%.2f)\n', mean(ranking_america), std(ranking_america));
fprintf('GLICA:\t\t%.2f (%.2f)\n', mean(ranking_GLICA), std(ranking_GLICA));
fprintf('QICA:\t\t%.2f (%.2f)\n', mean(ranking_QICA), std(ranking_QICA));
fprintf('SA4ICA:\t\t%.2f (%.2f)\n', mean(ranking_sa4ica), std(ranking_sa4ica));
